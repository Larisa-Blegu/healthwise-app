...
function AppointmentForDoctor() {
  ...
  const selectedProcedure = JSON.parse(
    localStorage.getItem("selectedProcedure")
  );
  ...
  const handleChangeCity = (e) => {
    ...
    fetch("http://localhost:8081/location/allLocations", {
      ...
      .then((data) => {
        ...
        const hospitals = filteredHospitals.map(
      ...
      .then((data) => {
        ...
        if (value) {
          filteredCity = filteredCity.filter(
            (location) => location.hospital === value
          );
        }
  ...
  const fetchLocation = async (hospital) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:8081/location/hospital/${hospital}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  async function save(
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        console.error(
          "Eroare la trimiterea cererii de programare:",
          response.statusText
        );
      }
    } catch (error) {
  ...
  return (
    <div>
      <div className="title">
        Cerere programare la {selectedDoctor.fullName}, pentru procedura{" "}
          ...
          {activeStep === 2 && (
            <div>
              ...
              <div className="response-message">
                <p>
                  Cererea dumneavoastră s-a trimis către medicul{" "}
                  {formData.doctor.fullName}. Veți fi notificat cu privire la
                  răspunsul său pe adresa de email: {formData.email} sau pe
                  pagina "Programarile tale"
                </p>
              </div>
            </div>
          )}
