...
function Doctor() {
  ...
  const uniqueSpecializations=[...new Set(specializations.map(specialization => specialization.name))];
  ...
  const handleDoctorSearch = (event) => {
    ...
    if (name !== null && name.trim() !== '') {
      axios.get(`http://localhost:8081/doctor/getDoctor/${name}`)
        .then(response => {
          setDoctors(response.data);
  ...
  const handleSpecializationSelect = (event) => {
    ...
    {
    const filteredDoctors = doctors.filter(doctor => {
        return doctor.specializations.some(specialization => specialization.name === name);
      });
  ...
  const handleReview = async (doctorId) => {
    try {
      const response = await axios.get(`http://localhost:8081/review/avg/${doctorId}`);
  ...
  const handleReviewCounter = async (doctorId) => {
    try {
      const response = await axios.get(`http://localhost:8081/review/counter/${doctorId}`);
      return response.data;
    } catch (error) {
      console.error('Eroare la încărcarea datelor:', error);
  ...
  return (
    <div>
      ...
      <div className="doctor-header">
        <h3>Medici</h3>
        <p>Căutare simplă, soluții sigure: Explorează o gamă variată de medici și specialiști, având astfel informațiile și resursele necesare pentru a lua cea mai bună decizie pentru sănătatea ta</p>
      </div>
    ...
    <option key={index} value={city}>{city}</option>
        ...
        <select value={selectedSpecialization} onChange={handleSpecializationSelect}>
    ...
    <option key={index} value={name}>{name}</option>
      ...
      <div className="doctor-list">
         ...
         <Link key={index} to={`/doctor/${doctor.id}`} className="doctor-card-link"> {/* Înfășoară cardul medicului într-un element Link */}
        ...
        {reviews[doctor.id] && (
            <div className='rating-container'>
                  <p className="rating-value">{reviews[doctor.id] !== undefined ? reviews[doctor.id].toFixed(2) : 'N/A'}</p>
                  <p>Recenzii: {reviewsCounter[doctor.id]}</p>
                  <Rating name={`rating-${doctor.id}`} value={reviews[doctor.id]} precision={0.1} readOnly />
            </div>           
                  )}    
    </div>
    </Link>
