...
function Specialization() {
  ...
  return (
    <div>
      ...
      <p className="SpecializationDescription">
        Fie că sunteți în căutarea unui specialist pentru o afecțiune specifică
        sau doriți să înțelegeți mai bine opțiunile disponibile, pagina noastră
        despre specializări medicale vă ajută să faceți alegeri informate în
        ceea ce privește îngrijirea sănătății dumneavoastră.
      </p>
      ...
      <div className="AccordeonAutocomplete">
        <div className="autocomplete-wrapper">
          <Autocomplete
            id="grouped-demo"
            options={specializations.sort((a, b) =>
              a.name.localeCompare(b.name)
            )}
            groupBy={(option) => option.name[0].toUpperCase()}
            getOptionLabel={(option) => option.name}
            renderInput={(params) => (
              <TextField {...params} label="Specializări" />
            )}
            renderGroup={(params) => (
              <li key={params.key}>
          ...
          {specializations.map((specialization, index) => (
            <div className="specializationDetails" key={index}>
              ...
              >
                ...
                >
                  <Typography variant="span" className="AccordionNumber">
                    {index + 1}
                  </Typography>
                ...
                <AccordionDetails className="AccordionDetails">
                  <div className="AboutButton">
                    <Link
                      to={`/DescriptionSpecialization/${specialization.id}`}
                      className="AboutLink"
                    >
                      Despre {specialization.name}
                    </Link>
                    <img
                      className="AboutPhoto"
                      src={specialization_icon}
                      alt={`Despre ${specialization.name}`}
                    />
                  </div>

                  <div className="AboutButton">
                    <Link
                      to={`/medicalProcedure/${specialization.id}`}
                      className="AboutLink"
                    >
                      Investigatii si proceduri medicale
                    </Link>
                    <img
                      className="AboutPhoto"
                      src={investigatii_icon}
                      alt={`Despre ${specialization.name}`}
                    />
                  </div>
                </AccordionDetails>
