...
function ManageDoctor() {
  ...
  const columns = [
    ...
    {
      field: "medicineDegree",
      headerName: "Titulatura",
      width: 200,
      editable: true,
      headerClassName: "super-app-theme--header",
    },
  ...
  const fetchSpecializations = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8081/specialization/allSpecializations",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      <Grid container direction="column" justifyContent="space-around">
        <Grid item xs={6} marginTop={2} marginLeft={3}>
          ...
          <Box marginBottom={4}>
            ...
            >
              Aceasta pagina este un panou de control care permite
              administratorilor să gestioneze informațiile despre medici în
              cadrul sistemului site-ului Healthwise. Aceasta oferă
              funcționalități pentru vizualiza și edita detaliile individuale
              ale fiecărui medic, adăugarea de noi medici și ștergerea medicilor
              care nu mai sunt necesari.{" "}
            </Typography>
          </Box>
        </Grid>
        <Backdrop
...
{dataLoaded && (
          <div style={{ marginRight: "40px", border: "1px solid #4c657f" }}>
            <ManagementDialog
              title={"Adauga Doctor-Utilizator"}
              tabelConnection={"Utilizatori"}
              operation={"Add"}
              initialDoctors={doctors}
              initialConnections={users}
              url="http://localhost:8081/user/doctor"  // Adjust based on your API endpoint
            />
          </div>
        )}
        ...
        {dataLoaded && (
          <div style={{ border: "1px solid #4c657f" }}>
            <ManagementDialog
              title={"Sterge Doctor-Utilizator"}
              tabelConnection={"Utilizatori"}
              operation={"Delete"}
              initialDoctors={doctors}
              initialConnections={users}
